...
GeoAlert.Markers = (function () {
  ...
  function getImpactRadii(event) {
    ...
    if (type === "earthquake") {
      ...
      return [
        {
          km: Math.max(15, Math.round(r1)),
          label: "Zona crítica",
          color: "#B91C1C",
          opacity: 0.25,
        },
        {
          km: Math.max(60, Math.round(r2)),
          label: "Daños moderados",
          color: "#EF4444",
          opacity: 0.15,
        },
        {
          km: Math.max(150, Math.round(r3)),
          label: "Sacudimiento leve",
          color: "#FB923C",
          opacity: 0.08,
        },
      ];
    ...
    if (type === "cyclone") {
      ...
      return [
        {
          km: Math.round(r * 0.4),
          label: "Ojo (vientos max)",
          color: "#7C3AED",
          opacity: 0.35,
        },
        {
          km: Math.round(r * 1.0),
          label: "Vientos huracanados",
          color: "#8B5CF6",
          opacity: 0.18,
        },
        {
          km: Math.round(r * 2.2),
          label: "Vientos destructivos",
          color: "#A78BFA",
          opacity: 0.09,
        },
      ];
    ...
    if (type === "flood") {
      return [
        { km: 20, label: "Inundación grave", color: "#0891B2", opacity: 0.3 },
        {
          km: 80,
          label: "Inundación parcial",
          color: "#06B6D4",
          opacity: 0.15,
        },
        {
  ...
  function createDisasterIcon(event) {
    ...
    const t =
      GeoAlert.CONFIG.DISASTER_TYPES[type] ||
      GeoAlert.CONFIG.DISASTER_TYPES.earthquake;
    const color = t.color;
    const speed =
      type === "tsunami" ? "1.4s" : type === "volcano" ? "1.0s" : "2.0s";
  ...
  function buildPopupHTML(event) {
    ...
    const typeInfo =
      GeoAlert.CONFIG.DISASTER_TYPES[event.type] ||
      GeoAlert.CONFIG.DISASTER_TYPES.earthquake;
    const radii = getImpactRadii(event);

    const impactRows = radii
      .map(
        (r) =>
  ...
  function createMarker(event) {
    ...
    const marker = L.marker([event.lat, event.lng], {
      icon,
      zIndexOffset: Math.round((event.magnitude || 0) * 100),
    ...
    marker.on("click", () => {
      ...
      impactCircles.forEach((c) => {
        c._geoVisible = true;
        updateCircleVisibility(c, GeoAlert.map.getZoom());
      });
    });
  ...
  function updateCircleVisibility(circle, zoom) {
    ...
    circle.setStyle({
      opacity: show ? circle.options.opacity : 0,
      fillOpacity: show ? circle.options.fillOpacity : 0,
    });
  }
